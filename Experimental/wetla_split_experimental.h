...
#define SPLIT_BUFFEREDDRAW		0x00000020
...
public:
	CDynSplitterImpl() :
			...
			m_cxySnapSize(1), m_hdcMem(NULL), m_hbmOld(NULL)
	{
...
public:
	...
	bool SetSplitterPos(int xyPos = -1, bool bUpdate = true)
	{
		...
		int cxyMax = IsSplitVertical() ? 
			m_rcSplitter.right - m_rcSplitter.left : m_rcSplitter.bottom - m_rcSplitter.top;
	...
	void DrawSplitter(CDCHandle dc)
	{
		...
		if (m_nSinglePane == SPLIT_PANE_NONE)
		{
			...
			for (int nPane = 0; nPane < m_nPanesCount; nPane++)
			{
				...
				{
					RECT rect;
					GetSplitterPaneRect(nPane, &rect);

					int nDC = ::SaveDC(m_hdcMem);
					::SetMapMode(m_hdcMem, MM_TEXT);

					POINT pt;
					::MapWindowPoints(m_hWndPane[nPane], m_hWnd, &rect, 2);
					::SetViewportOrgEx(m_hdcMem, -rect.left, -rect.top, &pt);

					HRGN hrgnPane = ::CreateRectRgn(rect.left, rect.top, rect.right, rect.bottom);
					::SelectClipRgn(m_hdcMem, hrgnPane);

					::SendMessage(m_hWndPane[nPane], WM_PRINTCLIENT, WPARAM(m_hdcMem),
								PRF_NONCLIENT | PRF_CLIENT | PRF_CHILDREN | PRF_ERASEBKGND);

					::SetViewportOrgEx(m_hdcMem, pt.x, pt.y, NULL);
					::RestoreDC(hdcDraw, nDC);
					::DeleteObject(HGDIOBJ(hrgnPane));
				}
			}
		}
...
public:
	...
	LRESULT OnPaint(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		if (wParam == NULL)
		{
			...
			if (m_hdcMem)
			{
				pT->DrawSplitter(m_hdcMem);
				::BitBlt(dc.m_hDC, m_rcSplitter.left, m_rcSplitter.top,
					m_rcSplitter.rigth - m_rcSplitter.left, m_rcSplitter.bottom - m_rcSplitter.top,
					m_hdcMem, m_rcSplitter.left, m_rcSplitter.top, SRCCOPY);
			}
			else pT->DrawSplitter(dc.m_hDC);
		}
		else
		{
			if (m_hdcMem)
			{
				pT->DrawSplitter(m_hdcMem);
				::BitBlt((HDC)wParam, m_rcSplitter.left, m_rcSplitter.top,
					m_rcSplitter.rigth - m_rcSplitter.left, m_rcSplitter.bottom - m_rcSplitter.top,
					m_hdcMem, m_rcSplitter.left, m_rcSplitter.top, SRCCOPY);
			}
			else pT->DrawSplitter((HDC)wParam);
		}
		return 0;
	}
	...
	LRESULT OnLButtonDown(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& bHandled)
	{
		...
		if (IsOverSplitterBar(xPos, yPos))
		{
			...
			else if (m_dwExtendedStyle & SPLIT_BUFFEREDDRAW)
			{
				HDC hdc;
				if (hdc = pT->GetDCEx(NULL, DCX_WINDOW | DCX_CACHE))
				{
					...
					if (hbmMem)
					{
						if (m_hdcMem = ::CreateCompatibleDC(hdc))
							m_hbmOld = (HBITMAP)::SelectObject(m_hdcMem, (HGDIOBJ)hbmMem);
						else ::DeleteObject(hbmMem);
					}
				}
			}
	...
	LRESULT OnCaptureChanged(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		else if (m_hdcMem)
		{
			if (m_hbmOld)
			{
				m_hbmOld = (HBITMAP)::SelectObject(m_hdcMem, (HGDIOBJ)m_hbmOld);
...
public:
		...
		if (m_nSinglePane == SPLIT_PANE_NONE)
		{
			...
			for (int nPane = 0; nPane < m_nPanesCount; nPane++)
			{
				if (GetSplitterPaneRect(nPane, &rect))
				{
					if (m_hWndPane[nPane] != NULL)
					{
						UINT uExtraFlags = (m_hdcMem) ? SWP_NOREDRAW : 0;

						if ((hdwpPanes = ::DeferWindowPos(hdwpPanes, m_hWndPane[nPane], NULL, 
							rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top,
							SWP_NOACTIVATE | SWP_NOZORDER | uExtraFlags)) == NULL)
							return;
					}
					else
						pT->InvalidateRect(&rect);
	...
	void GetSystemSettings(bool bUpdate)
	{
		...
		{
			m_cxyBarEdge = 0;
			m_cxyMin = 2 * ::GetSystemMetrics(bVertical ? SM_CXEDGE : SM_CYEDGE);
		}
